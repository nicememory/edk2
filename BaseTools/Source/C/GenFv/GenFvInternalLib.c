...
EFI_STATUS
UpdateMips64ResetVectorIfNeeded (
  IN MEMORY_FILE            *FvImage,
  IN FV_INFO                *FvInfo
  )
/*++
Routine Description:
  This parses the FV looking for SEC and patches that address into the
  beginning of the FV header.
  For Mips64 ISA, the reset vector is at 1fc00000.
Arguments:
  FvImage       Memory file for the FV memory image
  FvInfo        Information read from INF file.
Returns:
...
--*/
{
  ...
  if (MachineType == EFI_IMAGE_MACHINE_MIPS64) {
    UINT32 ResetVector[4];
    UINT32 InstrStack;
    memset(ResetVector, 0, sizeof (ResetVector));
    ...
    /* if we found an SEC core entry point then generate a branch instruction */
    if (UpdateVectorSec) {
      VerboseMsg("UpdateMipsResetVectorIfNeeded updating MIPS64 SEC vector");
...
--*/
{
  ...
  if ((*MachineType != EFI_IMAGE_MACHINE_IA32) &&  (*MachineType != EFI_IMAGE_MACHINE_X64) && (*MachineType != EFI_IMAGE_MACHINE_EBC) &&
      ...
      (*MachineType != EFI_IMAGE_MACHINE_RISCV64) && (*MachineType != EFI_IMAGE_MACHINE_MIPS64)) {
...
--*/
{
  ...
  if (mMips64) {
    Status = UpdateMips64ResetVectorIfNeeded (&FvImageMemoryFile, &mFvDataInfo);
    if (EFI_ERROR (Status)) {
      Error (NULL, 0, 3000, "Invalid", "Could not update the reset vector.");
      goto Finish;
    }
...
--*/
{
  ...
  for (Index = 1;; Index++) {
    ...
    if (!EFI_ERROR(Status)) {
      ...
      // machine type is MIPS64, set a flag so MIPS64 reset vector procesing occurs
      if ((MachineType == EFI_IMAGE_MACHINE_MIPS64)) {
        VerboseMsg("Located MIPS64 SEC core in child FV");
        mMips64 = TRUE;
      }
    }
...
--*/
{
  ...
  for (Index = 1;; Index++) {
    ...
    if ((ImageContext.Machine == EFI_IMAGE_MACHINE_MIPS64)) {
      mMips64 = TRUE;
    }
