    ...
    if (RelShdr->sh_type == SHT_RELA && (*Filter)(SecShdr)) {
      ...
      for (RelIdx = 0; RelIdx < RelShdr->sh_size; RelIdx += (UINT32) RelShdr->sh_entsize) {
        ...
        if (Sym->st_shndx == SHN_UNDEF
          ...
          // from RISC-V or LoongArch toolchain.
          //
          ...
          if (mEhdr->e_machine == EM_X86_64) {
            ...
            switch (ELF_R_TYPE(Rel->r_info)) {
              case R_LARCH_MARK_LA:
                CoffAddFixup (mCoffSectionsOffset[RelShdr->sh_info] + (Rel->r_offset - SecShdr->sh_addr), EFI_IMAGE_REL_BASED_LOONGARCH64_MARK_LA);
                break;
              case R_LARCH_64:
                CoffAddFixup(
                  (UINT32) ((UINT64) mCoffSectionsOffset[RelShdr->sh_info]
